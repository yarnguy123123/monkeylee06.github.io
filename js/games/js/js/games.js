const games = {
  ...
  yarnDodge: {
    ...
    start(canvas, ctx) {
      ...
      const loop = () => {
        ...
        if (this.keys["ArrowUp"]) player.y -= player.speed;
        if (this.keys["ArrowDown"]) player.y += player.speed;
  ...
  yarnClicker: {
    ...
    start(canvas, ctx) {
      ...
      const center = { x: canvas.width / 2, y: canvas.height / 2, r: 80 };
      ...
      const loop = () => {
        ...
        ctx.fillText("Click!", center.x, center.y + 8);

        ctx.fillStyle = "#ffddff";
        ctx.font = "22px Segoe UI";
        ctx.fillText(`Clicks: ${this.clicks}`, center.x, center.y + 60);
  ...
  yarnJump: {
    ...
    start(canvas, ctx) {
      ...
      const player = { x: canvas.width / 2, y: canvas.height - 80, vy: 0, r: 18 };
      const gravity = 0.5;
      const jump = -12;
      ...
      const loop = () => {
        ...
        if (this.keys["ArrowLeft"]) player.x -= 4;
        if (this.keys["ArrowRight"]) player.x += 4;
        ...
        platforms = platforms.filter(p => p.y < canvas.height + 40);
  ...
  yarnShooter: {
    ...
    start(canvas, ctx) {
      ...
      const player = { x: canvas.width / 2, y: canvas.height - 70, w: 30, h: 30 };
      let bullets = [];
      let enemies = [];
      let frame = 0;
      ...
      const loop = () => {
        if (!this.running) {
          window.removeEventListener('keydown', keyDown);
          window.removeEventListener('keyup', keyUp);
          window.removeEventListener('keydown', shootHandler);
          return;
        }
        ...
        if (this.keys["ArrowLeft"]) player.x -= 5;
        if (this.keys["ArrowRight"]) player.x += 5;
        ...
        enemies = enemies.filter(en => en.y < canvas.height + 40);
